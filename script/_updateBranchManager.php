...
$success = 0;
$error = [];
$userid    = $_REQUEST['editBranchManagerid'];
...
if($v->passes() && $img_err == "" && $documents_err == "" && $date_err == "") {
  if($img['size'] != 0){
    ...
    $sql = "select * from staff where id = ? and role_id=4";
  ...
  $sql =     'update staff set end_date=?,branch_id=?,name=?,phone=?,email=?,address=?,salary=? where id=?';
  $result = setData($con,$sql,[$end,$branch,$name,$phone,$email,$address,$salary,$userid]);
  if($result > 0){
    $success = 1;
  }
