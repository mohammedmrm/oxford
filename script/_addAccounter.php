...
if($v->passes() && $img_err == "" && $documents_err == "" && $date_err == "") {
  ...
  $sql = 'insert into staff (end_date,name,phone,email,address,salary,img,documents,password,role_id) values
                             (?,?,?,?,?,?,?,?,?,?)';
  $result = setData($con,$sql,[$end,$name,$phone,$email,$address,$salary,$imgPath,
                               $documentsPath,$password,"2"]);
  if($result > 0){
    $success = 1;
  }
}else{
