<div class="kt-portlet kt-portlet--mobile">
	<div class="kt-portlet__body">
        <button type="button" data-toggle="modal" data-target="#addHr"class="btn btn-info text-center btn-lg"  >اضافة مدير موارد بشرية</button>
	</div>
</div>
<div class="kt-portlet kt-portlet--mobile">
    ...
    <div class="kt-portlet__body">
		<!--begin: Datatable -->
		<table class="table table-striped- table-bordered table-hover table-checkable" id="tb_Hrs">
    		<thead>
        		<tr>
                  <th>ID</th>
                  <th>الاسم</th>
...
<div class="modal fade" id="addHr" role="dialog">
    <div class="modal-dialog">
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header" dir="ltr">
          <button type="button" class="close" data-dismiss="modal"></button>
          <h4 class="modal-title">اضافة مدير موارد بشرية</h4>
        </div>
        <div class="modal-body">
		<!--begin::Portlet-->
			<!--begin::Form-->
			<form class="kt-form" id="addHrForm">
				<div class="kt-portlet__body">
					<div class="kt-section kt-section--first">
						...
						<div class="form-group">
							...
							<input type="date" class="form-control" placeholder="" id="end" name="end">
	            ...
	            <div class="kt-portlet__foot">
					<div class="kt-form__actions">
						<button type="button" onclick="addHr()" class="btn btn-primary">اضافة</button>
...
<div class="modal fade" id="editHr" role="dialog">
    <div class="modal-dialog">
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header" dir="ltr">
          <button type="button" class="close" data-dismiss="modal"></button>
          <h4 class="modal-title">تعديل معلومات مدير الموارد البشرية</h4>
        </div>
        <div class="modal-body">
		<!--begin::Portlet-->
			<!--begin::Form-->
			<form class="kt-form" id="updateHrForm">
				<div class="kt-portlet__body">
					<div class="kt-section kt-section--first">
						<div class="form-group">
							<label>الاسم الكامل:</label>
	            ...
	            <div class="kt-portlet__foot">
					<div class="kt-form__actions">
						<button type="button" onclick="updateHr()" class="btn btn-primary">حفظ التعديلات</button>
						<button type="reset" class="btn btn-secondary">الغأ</button>
					</div>
				</div>
                <input type="hidden" name="editHrid" id="editHrid"/>
			</form>
			<!--end::Form-->
		<!--end::Portlet-->
        </div>
...
function getHrs(){
$.ajax({
  url:"script/_getHrs.php",
  type:"POST",
  beforeSend:function(){
    $("#Hrs").addClass('loading');
  },
  success:function(res){
   $("#tb_Hrs").DataTable().destroy();
   console.log(res);
   $("#Hrs").html("");
   $("#Hrs").removeClass('loading');
   $.each(res.data,function(){
     $("#Hrs").append(
       '<tr>'+
            ...
            '<td>'+this.note+'</td>'+
            '<td width="150px">'+
              '<button class="btn btn-clean btn-icon-lg" onclick="editHr('+this.id+')" data-toggle="modal" data-target="#editHr"><span class="flaticon-edit"></sapn>'+
              '<button class="btn btn-clean btn-icon-lg" onclick="deleteHr('+this.id+')" data-toggle="modal" data-target="#deleteHr"><span class="flaticon-delete"></sapn>'+
            '</button></td>'+

       '</tr>');
     });
     var myTable= $('#tb_Hrs').DataTable({
     columns:[
    //"dummy" configuration
        { visible: true }, //col 1
        { visible: true }, //col 2
...
function addHr(){
    var myform = document.getElementById('addHrForm');
    var fd = new FormData(myform);
  $.ajax({
    url:"script/_addHr.php",
    type:"POST",
    data:fd,
    ...
    beforeSend:function(){
     ...
     $("#end_err").text('');
     $("#img_err").text('');
     $("#addHrForm").addClass("loading");
    },
    success:function(res){
        $("#addHrForm").removeClass("loading");
       console.log(res);
       if(res.success == 1){
         $("#name").val("");
         $("#phone").val("");
...
function updateHr(){
    var myform = document.getElementById('updateHrForm');
    var fd = new FormData(myform);
  $.ajax({
    url:"script/_updateHr.php",
    type:"POST",
    data:fd,
    processData: false,  // tell jQuery not to process the data
    contentType: false,
    ...
    beforeSend:function(){
     ...
     $("#e_img_err").text('');
     $("#e_end_err").text('');
     $("#updateHrForm").addClass("loading");
    },
    success:function(res){
       $("#updateHrForm").removeClass("loading");
       console.log(res);
       if(res.success == 1){
         $("#e_name").val("");
         $("#e_phone").val("");
