<div class="kt-portlet kt-portlet--mobile">
	<div class="kt-portlet__body">
        <button type="button" data-toggle="modal" data-target="#addBranchManager"class="btn btn-warning text-center btn-lg"  >اضافة مدير فرع</button>
	</div>
</div>
<div class="kt-portlet kt-portlet--mobile">
    ...
    <div class="kt-portlet__body">
		<!--begin: Datatable -->
		<table class="table table-striped- table-bordered  responsive no-wrap" id="tb_BranchManagers">
    		<thead>
        		<tr>
                  <th>ID</th>
                  <th>الفرع</th>
            ...
            <tbody id="BranchManagers">
            </tbody>
        </table>
		<!--end: Datatable -->
	</div>
</div>
<div class="modal fade" id="addBranchManager" role="dialog">
    <div class="modal-dialog">
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header" dir="ltr">
          <button type="button" class="close" data-dismiss="modal"></button>
          <h4 class="modal-title">اضافة مدير فرع</h4>
        </div>
        <div class="modal-body">
		<!--begin::Portlet-->
			<!--begin::Form-->
			<form class="kt-form" id="addBranchManagerForm">
				<div class="kt-portlet__body">
	            ...
	            <div class="kt-portlet__foot">
					<div class="kt-form__actions">
						<button type="button" onclick="addBranchManager()" class="btn btn-primary">اضافة</button>
...
<div class="modal fade" id="editBranchManager" role="dialog">
    <div class="modal-dialog">
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header" dir="ltr">
          <button type="button" class="close" data-dismiss="modal"></button>
          <h4 class="modal-title">تعديل معلومات مدير الفرع</h4>
        </div>
        <div class="modal-body">
		<!--begin::Portlet-->
			<!--begin::Form-->
			<form class="kt-form" id="updateBranchManagerForm">
				<div class="kt-portlet__body">
					<div class="kt-section kt-section--first">
						<div class="form-group">
							<label>الاسم الكامل:</label>
						...
						<div class="form-group">
							<label>الفرع:</label>
                            <select id="e_branch" name="e_branch" class="form-control"></select>
	            ...
	            <div class="kt-portlet__foot">
					<div class="kt-form__actions">
						<button type="button" onclick="updateBranchManager()" class="btn btn-primary">حفظ التعديلات</button>
						<button type="reset" class="btn btn-secondary">الغأ</button>
					</div>
				</div>
                <input type="hidden" name="editBranchManagerid" id="editBranchManagerid"/>
			</form>
			<!--end::Form-->
		<!--end::Portlet-->
        </div>
...
function getBranchManagers(){
$.ajax({
  url:"script/_getBranchManagers.php",
  type:"POST",
  beforeSend:function(){
    $("#BranchManagers").addClass('loading');
  },
  success:function(res){
   $("#tb_BranchManagers").DataTable().destroy();
   console.log(res);
   $("#BranchManagers").html("");
   $("#BranchManagers").removeClass('loading');
   $.each(res.data,function(){
     $("#BranchManagers").append(
       '<tr>'+
            ...
            '<td>'+this.note+'</td>'+
            '<td width="150px">'+
              '<button class="btn btn-clean btn-icon-lg" onclick="editBranchManager('+this.id+')" data-toggle="modal" data-target="#editBranchManager"><span class="flaticon-edit"></sapn>'+
              '<button class="btn btn-clean btn-icon-lg" onclick="deleteBranchManager('+this.id+')" data-toggle="modal" data-target="#deleteBranchManager"><span class="flaticon-delete"></sapn>'+
            '</button></td>'+

       '</tr>');
     });
     var myTable= $('#tb_BranchManagers').DataTable({
     columns:[
    //"dummy" configuration
        { visible: true }, //col 1
        { visible: true }, //col 2
...
function addBranchManager(){
    var myform = document.getElementById('addBranchManagerForm');
    var fd = new FormData(myform);
  $.ajax({
    url:"script/_addBranchManager.php",
    type:"POST",
    data:fd,
    ...
    beforeSend:function(){
     ...
     $("#end_err").text('');
     $("#img_err").text('');
     $("#addBranchManagerForm").addClass("loading");
    },
    success:function(res){
        $("#addBranchManagerForm").removeClass("loading");
       console.log(res);
       if(res.success == 1){
         $("#name").val("");
         $("#phone").val("");
...
function editBranchManager(id){
  $("#editBranchManagerid").val(id);

  $.ajax({
    url:"script/_getBranchManager.php",
    data:{id: id},
    success:function(res){
      if(res.success == 1){
        $.each(res.data,function(){
...
function updateBranchManager(){
    var myform = document.getElementById('updateBranchManagerForm');
    var fd = new FormData(myform);
  $.ajax({
    url:"script/_updateBranchManager.php",
    type:"POST",
    data:fd,
    processData: false,  // tell jQuery not to process the data
    contentType: false,
    ...
    beforeSend:function(){
     ...
     $("#e_img_err").text('');
     $("#updateBranchManagerForm").addClass("loading");
    },
    success:function(res){
       $("#updateBranchManagerForm").removeClass("loading");
       console.log(res);
       if(res.success == 1){
         $("#e_name").val("");
         $("#e_phone").val("");
...
function deleteBranchManager(id){
  if(confirm("هل انت متاكد من الحذف")){
      $.ajax({
        url:"script/_deleteBranchManager.php",
        type:"POST",
        data:{id:id},
        success:function(res){
         if(res.success == 1){
